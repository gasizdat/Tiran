...
function GameArea() {
    ...
    this.savedStatesList = function () {
        var ret = [];
        var count = CookieHelper.getCookie(_SGC_NAME);
        if (!count)
            count = 0;
        else
    ...
    function _saveCurrentLevelState() {
        if (_currentLevel) {
            ...
            CookieHelper.setCookie(_S_NAME + count, value, _cookieElapsedTime);
            if (CookieHelper.getCookie(_S_NAME + count)) {
                _msgFrame.show(StringResources.GAME_SUCCESS_SAVED, true, "green");
                CookieHelper.setCookie(_SGC_NAME, ++count, _cookieElapsedTime);
            }
            else
